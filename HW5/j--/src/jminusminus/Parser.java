...
public class Parser {
    ...
    private boolean seeBasicType() {
        if (see(BOOLEAN) || see(CHAR) || see(INT)|| see(DOUBLE)|| see (LONG)) {
            return true;
        } else {
            return false;
        }
    ...
    private boolean seeLabeledStatement() {
        scanner.recordPosition();
...
/////////////// Interface /////////////
    ...
    private JMember interfaceMemberDecl(ArrayList<String> mods) {
        ...
        if (seeIdentLParen()) {
            ...
            JBlock body = block();
            interfaceMemberDecl = new JConstructorDeclaration(line, mods, name, params,
                    body);
        } 
        else if (see(CLASS)){ //for inner class
        	interfaceMemberDecl = classDeclaration(mods);
        }
        else if (see(INTERFACE)){ //for inner interface
    ...
    private JMember memberDecl(ArrayList<String> mods) {
        ...
        if (seeIdentLParen()) {
            ...
            else if (have(LCURLY) && mods.size() == 1 && mods.contains(STATIC.image())) {  
			    JBlock body = have(SEMI) ? null : block();
			    memberDecl = new JStaticBlock(line, mods, body);
		    } 
            else if (see(CLASS)){ //inner class
        	    memberDecl = classDeclaration(mods);
            }
            else if (see(INTERFACE)){ //inner class
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            ...
            return new JForStatement(line, forInit, expression, forUpdate, statements);
            
        } else if (have(DO)) {
        	JStatement statements = statement();
        	mustBe(WHILE);
        	JExpression test = parExpression(); 
        	mustBe(SEMI);
            return new JDoWhileStatement(line, statements, test);

        } else if (have(TRY)) {
            JBlock attempt = block();
            ArrayList<JCatch> catches = new ArrayList<>();
            JBlock finish = null;
            ...
            while(have(CATCH)) {
                ...
                JFormalParameter exception = new JFormalParameter(catchLine, name, type);
                mustBe(RPAREN);
                JBlock caught = block();
			...
			return new JTryStatement(line, attempt, catches, finish);
            ...
            return new JEmptyStatement(line);
             ...
             ArrayList<JSwitchBlockStatement> switchBlockStatementGroup = new ArrayList<JSwitchBlockStatement>();
             mustBe(LCURLY);
                while(!see(RCURLY) && !see(EOF)) {
                    if (have(BREAK)) {
                        mustBe(SEMI);
...
//////  SwitchBlockStatement    //////
    private JSwitchBlockStatement switchBlockStatementGroup() {
    	...
    	return new JSwitchBlockStatement(line, switchLabels, block);
    }
...
//////  ForInit //////
    private ArrayList<JStatement> forInit(int line) {
    	...
    	if (seeBasicType() || seeReferenceType()) {
            ...
            forInit.add(new JVariableDeclaration(line, mods, vdecls));
        } else {
...
///// ForUpdate //////
    ...
    private JExpression assignmentExpression() {
        ...
        if (have(ASSIGN)) {
            ...
            return new JUnsignRShiftAssignOp(line, lhs, assignmentExpression());
        } else if (have(ALS_ASSIGN)) {
            return new JLShiftAssignOp(line, lhs, assignmentExpression());
        } else if (have(BXOR_ASSIGN)) {
            return new JCaretAssignOp(line, lhs, assignmentExpression());
...
// conditional Expression (e1 ? e2 : e3)       Level 12
    private JExpression conditionalExpression() {
        int line = scanner.token().line();
        JExpression lhs = conditionalOrExpression();
        if (have(QUESTION)) {
            JExpression firstRHS = assignmentExpression();
            mustBe(COLON);
            JExpression secondRHS = conditionalExpression();
            return new JConditionalExpression(line, lhs, firstRHS, secondRHS);
        } else {
            return lhs;
        }
    }
...
//  conditional And Expression ||       Level 11
...
// level 9 inclusiveOrExpression ::= exclusiveOrExpression { BOR exclusiveOrExpression }
    private JExpression inclusiveOrExpression() {
        ...
        while (more) {
            if (have(BOR)) {
                lhs = new JBorOP(line, lhs, exclusiveOrExpression());
            } else {
                more = false;
            }
        }
 ...
 // level 8 exclusiveOrExpression ::= andExpression { BXOR andExpression } 
    ...
    private JExpression exclusiveOrExpression() {
        ...
        while (more) {
            if (have(BXOR)) {
                lhs = new JBxorOP(line, lhs, andExpression());
            } else {
                more = false;
            }
        }
   ...
   // level 7 andExpression ::= equalityExpression { BAND equalityExpression }
    ...
    private JExpression equalityExpression() {
        ...
        while (more) {
            ...
            } else if (have (LNOT_EQUAL)){
                lhs =  new JLogicalNotEqualOp (line, lhs, relationalExpression());
            }else {
                more = false;
            }
        }
    ...
    private JExpression simpleUnaryExpression() {
        ...
        if (have(LNOT)) {
            ...
            return new JBnotOp(line, unaryExpression());        
        }  else if (seeCast()) {
            mustBe(LPAREN);
