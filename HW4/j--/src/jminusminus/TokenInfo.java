...
enum TokenKind {
    ...
    PLUS("+"), PLUS_ASSIGN("+="),ASSIGN("="), INC("++"),DEC("--"), EQUAL("=="), GT(">"), GE(">="),
    LT("<"), LE("<="), LNOT("!"),LNOT_EQUAL("!="), MINUS("-"), MIN_ASSIGN("-="), STAR("*"),
    STAR_ASSIGN("*="), DIV("/"), DIV_ASSIGN("/="), REM("%"),REM_ASSIGN("%="), ALS("<<"),
    ALS_ASSIGN("<<="), ARS(">>"), ARS_ASSIGN(">>="),LRS(">>>"), LRS_ASSIGN(">>>="),BNOT("~"),
    BOR("|"), BOR_ASSIGN("|="), BXOR("^"),BXOR_ASSIGN("^="),BAND("&"),BAND_ASSIGN("&="),LAND("&&"),
    LOR("||"),

    //Separators
    LPAREN("("), RPAREN(")"), LCURLY("{"), RCURLY("}"), LBRACK("["),RBRACK("]"), SEMI(";"), 
    COMMA(","), COLON(":"), QUESTION("?"), DOT("."), ;
