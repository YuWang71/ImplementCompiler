 ...
 * 
 * When you add a new token to the scanner, you must also add an entry
 * to this enum specifying the kind and image of the new token.
 */

enum TokenKind {
    //Reserve Words.
    EOF("<EOF>"), ABSTRACT("abstract"), BOOLEAN("boolean"), CHAR("char"), CLASS("class"), ELSE("else"),
    EXTENDS("extends"), FALSE("false"), IF("if"),IMPORT("import"), INSTANCEOF("instanceof"), INT("int"),
    NEW("new"), NULL("null"), PACKAGE("package"), PRIVATE("private"), PROTECTED("protected"), 
    PUBLIC("public"),RETURN("return"), STATIC("static"), SUPER("super"), THIS("this"), TRUE("true"), 
    VOID("void"), WHILE("while"), BREAK("break"),CASE("case"), CATCH("catch"),CONTINUE("continue"),  
    DEFAULT("default"), DO("do"),  DOUBLE("double"), FINAL("final"), FINALLY("finally"), FOR("for"), 
    IMPLEMENTS("implements"), INTERFACE("interface"), LONG("long"), SWITCH("switch"), THROW("throw"), 
    THROWS("throws"), TRY("try"),IDENTIFIER("<IDENTIFIER>"), INT_LITERAL("<INT_LITERAL>"), 
    CHAR_LITERAL("<CHAR_LITERAL>"), STRING_LITERAL("<STRING_LITERAL>"), 
    DOUBLE_LITERAL("<DOUBLE_LITERAL>"), LONG_LITERAL("<LONG_LITERAL>"),
     //Operators, separators
    PLUS("+"), ASSIGN("="), DEC("--"), EQUAL("=="), GT(">"), INC("++"), LAND("&&"), LE("<="), 
    LNOT("!"), MINUS("-"), PLUS_ASSIGN("+="), STAR("*"), DIV("/"), REM("%"),ALS ("<<"),ARS(">>"),
    LRS(">>>"), BNOT("~"),BOR("|"), BXOR("^"),BAND("&"),LPAREN("("), RPAREN(")"), LCURLY("{"), 
    RCURLY("}"), LBRACK("["),RBRACK("]"), SEMI(";"), COMMA(","), COLON(":"), QUESTION("?"),LNOT_EQUAL("!="),
    DIV_EQUAL("/="),MIN_EQUAL("-="), MULTI_EQUAL("*="),REM_EQUAL("%="), ARS_EQUAL(">>="), 
    LRS_EQUAL(">>>="),GT_EQUAL(">="),ALS_EQUAL("<<="),LESS("<"), POW_EQUAL("^="), BOR_EQUAL("|="),
    LOR("||"),LOR_EQUAL("||="), BAND_EQUAL("&="),DOT("."), ;
